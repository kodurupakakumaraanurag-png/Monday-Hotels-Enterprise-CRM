...
export function GlobalSearchModal({ isOpen, onClose }: GlobalSearchModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 bg-[#1E293B]/60 backdrop-blur-md p-4 overflow-y-auto">
      <div className="bg-white border border-[#E5E2D9] rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden text-[#1E293B] flex flex-col max-h-[80vh]">
        {/* Search Input Bar */}
        <div className="p-4 border-b border-[#E5E2D9] flex items-center space-x-3 bg-[#F7F4EC]">
          ...
          <input
            ...
            className="w-full bg-transparent text-sm text-[#1E293B] placeholder:text-[#6B766F] focus:outline-none"
          />
          {isSearching ? (
            <Loader2 className="w-4 h-4 text-[#285943] animate-spin shrink-0" />
          ) : (
            query && (
              <button onClick={() => setQuery("")} className="text-[#6B766F] hover:text-[#1E293B]">
                <X className="w-4 h-4" />
              </button>
            )
          )}
          <button
            onClick={onClose}
            className="px-2 py-1 bg-[#DDE9E1] hover:bg-[#A8C3B2] text-[#1E4D3B] rounded-lg text-xs font-mono font-bold"
          >
            ESC
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5 bg-white">
          {!query.trim() ? (
            ...
            <div className="py-10 text-center space-y-3">
              ...
              <div className="text-sm font-bold text-[#1E293B]">No matching CRM records found</div>
              <p className="text-xs text-[#6B766F] max-w-sm mx-auto">
                No results for &quot;<strong className="text-[#1E293B]">{query}</strong>&quot;. Try checking POC names, emails, company codes, or reservation numbers.
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs text-[#6B766F] px-1 border-b border-[#E5E2D9] pb-2">
                <span>
                  Found <strong className="text-[#1E293B]">{searchResults.totalCount}</strong> matching record{searchResults.totalCount > 1 ? "s" : ""}
                </span>
                          ...
                          return (
                            <div
                              ...
                              className={`p-3 rounded-xl border transition cursor-pointer flex items-center justify-between gap-3 ${
                                isSelected
                                  ? "bg-[#DDE9E1] border-[#285943] text-[#1E4D3B]"
                            ...
                            >
                              <div className="flex items-center space-x-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <div className="text-xs font-bold truncate text-[#1E293B] flex items-center space-x-2">
                                    <span>{item.title}</span>
                                    {item.badge && (
                                      <span
                                        className={`px-2 py-0.5 text-[9px] font-bold rounded ${
                                          item.badgeColor || "bg-[#F7F4EC] text-[#1E293B] border border-[#E5E2D9]"
                                        }`}
                                      >
                                        {item.badge}
                                      </span>
                              ...
                              <div className="flex items-center space-x-3 shrink-0 text-right">
                                {item.metadataText && (
                                  <span className="text-xs font-bold text-[#1E293B] hidden sm:inline-block">
                                    {item.metadataText}
                                  </span>
                                )}
                                <ArrowRight
        ...
        <div className="p-3 bg-[#F7F4EC] border-t border-[#E5E2D9] flex items-center justify-between text-[11px] text-[#6B766F]">
          <div className="flex items-center space-x-4">
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 bg-white border border-[#E5E2D9] rounded font-mono text-[10px] text-[#1E293B]">↑↓</kbd> Navigate
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 bg-white border border-[#E5E2D9] rounded font-mono text-[10px] text-[#1E293B]">↵</kbd> Select
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 bg-white border border-[#E5E2D9] rounded font-mono text-[10px] text-[#1E293B]">ESC</kbd> Close
            </span>
          </div>
